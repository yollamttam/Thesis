...
At this point, we have an expression for the \lya\ optical depth in terms of several properties of the IGM. The primary unknown in the above expression is the photoionization rate, which depends on the number, intensity, and proximity of ionizing sources. A common assumption in these types of analyses is that the photoionization rate is approximately spatially uniform. This seems like a plausible assumption in a post-reionization Universe, when any given gas parcel can receive ionizing radiation from hundreds of sources. With this approximation, the observed mean transmission in a region of the spectrum is akin to an average of Eq. \ref{eq:tauGPfinal} over the density field:
...
\cite{fan2002evolution}


We probably want to mention in this section things about how quasars are in special zones and are more likely to have purely ionized lines of sight than random lines of sight through the Universe. 
